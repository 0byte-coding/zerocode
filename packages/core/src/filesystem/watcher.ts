...
import { FileSystemWatcher } from "@0codeai/zerocode-schema/filesystem-watcher"
...
declare const ZEROCODE_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof ZEROCODE_LIBC === "undefined" ? undefined : ZEROCODE_LIBC
...
const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.ZEROCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.ZEROCODE_EXPERIMENTAL_FILEWATCHER)) {
