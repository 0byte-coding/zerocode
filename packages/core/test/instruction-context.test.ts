...
import { InstructionContext } from "@0codeai/zerocode-core/instruction-context"
...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.ZEROCODE_DISABLE_PROJECT_CONFIG
      let scanned = false
      process.env.ZEROCODE_DISABLE_PROJECT_CONFIG = "1"
      ...
      yield* SystemContextRegistry.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.ZEROCODE_DISABLE_PROJECT_CONFIG
            else process.env.ZEROCODE_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
