...
import { ModelsDevPlugin } from "@0codeai/zerocode-core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.ZEROCODE_MODELS_PATH,
          disabled: Flag.ZEROCODE_DISABLE_MODELS_FETCH,
        }
        Flag.ZEROCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.ZEROCODE_MODELS_PATH = previous.path
          Flag.ZEROCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
