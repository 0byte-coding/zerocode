...
import { PluginHost } from "@0codeai/zerocode-core/plugin/host"
import { OpencodePlugin } from "@0codeai/zerocode-core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ ZEROCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses ZEROCODE_API_KEY as credentials", () =>
    withEnv({ ZEROCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ ZEROCODE_API_KEY: undefined, CUSTOM_ZEROCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_ZEROCODE_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
