...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export ZEROCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export ZEROCODE_CLIENT=desktop",
    `export ZEROCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export ZEROCODE_SERVER_PASSWORD=${shellEscape(password)}`,
