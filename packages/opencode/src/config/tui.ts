...
import { TuiKeybind } from "@0codeai/zerocode-tui/config/keybind"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and ZEROCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ZEROCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ZEROCODE_TUI_CONFIG override, if set.
  if (Flag.ZEROCODE_TUI_CONFIG) {
    const configFile = Flag.ZEROCODE_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and ZEROCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.ZEROCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.ZEROCODE_CONFIG_DIR) continue
      ...
      (dir) =>
        npm
          .install(dir, {
            add: [
              {
                // Real, published npm package — see config.ts for why this stays
                // pointed at the actual registry name.
