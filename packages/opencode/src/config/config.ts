...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.ZEROCODE_CONFIG && !Flag.ZEROCODE_CONFIG_DIR && !Flag.ZEROCODE_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ZEROCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.ZEROCODE_CONFIG) {
          yield* merge(Flag.ZEROCODE_CONFIG, yield* loadFile(Flag.ZEROCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.ZEROCODE_CONFIG })
        }

        if (!Flag.ZEROCODE_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.ZEROCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ZEROCODE_CONFIG_DIR", { path: Flag.ZEROCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.ZEROCODE_CONFIG_DIR) {
        ...
        if (process.env.ZEROCODE_CONFIG_CONTENT) {
          const source = "ZEROCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ZEROCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ZEROCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ZEROCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ZEROCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ZEROCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ZEROCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("ZEROCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.ZEROCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ZEROCODE_DISABLE_PRUNE) {
