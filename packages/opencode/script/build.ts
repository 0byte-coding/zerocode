...
const createEmbeddedWebUIBundle = async () => {
  ...
  await $`ZEROCODE_CHANNEL=${Script.channel} bun run --cwd ${appDir} build`
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    files: embeddedFileMap ? { "zerocode-web-ui.gen.ts": embeddedFileMap } : {},
    entrypoints: ["./src/index.ts", parserWorker, workerPath, ...(embeddedFileMap ? ["zerocode-web-ui.gen.ts"] : [])],
    define: {
      FFF_LIBC: JSON.stringify(item.abi === "musl" ? "musl" : "gnu"),
      ZEROCODE_VERSION: `'${Script.version}'`,
      ZEROCODE_MODELS_DEV: generated.modelsData,
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
      ZEROCODE_WORKER_PATH: workerPath,
      ZEROCODE_CHANNEL: `'${Script.channel}'`,
      ZEROCODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
