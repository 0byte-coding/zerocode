...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.ZEROCODE_CONFIG
  delete process.env.ZEROCODE_TUI_CONFIG
...
it.instance("project config takes precedence over ZEROCODE_TUI_CONFIG (matches ZEROCODE_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("ZEROCODE_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from ZEROCODE_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
