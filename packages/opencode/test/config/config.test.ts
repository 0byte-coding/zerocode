...
const managedConfigDir = process.env.ZEROCODE_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.ZEROCODE_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.ZEROCODE_CONSOLE_TOKEN
  else process.env.ZEROCODE_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
...
it.effect("does not create global config when ZEROCODE_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ZEROCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.effect("does not try to install dependencies in read-only ZEROCODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("ZEROCODE_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible ZEROCODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("ZEROCODE_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing ZEROCODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable ZEROCODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "ZEROCODE_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
// Note: preload.ts sets ZEROCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("ZEROCODE_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { ZEROCODE_CONFIG_DIR: undefined, ZEROCODE_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "ZEROCODE_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { ZEROCODE_DISABLE_PROJECT_CONFIG: "true", ZEROCODE_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed ZEROCODE_PERMISSION JSON used to crash
...
describe("ZEROCODE_PERMISSION env var", () => {
  it.instance("does not crash when ZEROCODE_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "ZEROCODE_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("ZEROCODE_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in ZEROCODE_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "ZEROCODE_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in ZEROCODE_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
