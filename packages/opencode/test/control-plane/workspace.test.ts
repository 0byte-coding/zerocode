...
import { WorkspaceTable } from "@0codeai/zerocode-core/control-plane/workspace.sql"
...
const originalEnv = {
  ZEROCODE_AUTH_CONTENT: process.env.ZEROCODE_AUTH_CONTENT,
  ZEROCODE_EXPERIMENTAL_WORKSPACES: process.env.ZEROCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.ZEROCODE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.ZEROCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.ZEROCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.ZEROCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.ZEROCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
