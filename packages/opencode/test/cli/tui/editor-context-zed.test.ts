...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@0codeai/zerocode-tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.ZEROCODE_ZED_DB
  process.env.ZEROCODE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.ZEROCODE_ZED_DB
    else process.env.ZEROCODE_ZED_DB = previous
    home.mockRestore()
  }
})
