...
async function translate(
  ...
  env.ZEROCODE_DISABLE_PROJECT_CONFIG = "1"
  env.ZEROCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["zerocode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.ZEROCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["zerocode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.ZEROCODE_CONFIG
  delete env.ZEROCODE_CONFIG_DIR
  delete env.ZEROCODE_CONFIG_CONTENT
  delete env.ZEROCODE_PERMISSION
  delete env.ZEROCODE_AUTO_SHARE
  return env
}
