...
```text
packages/opencode ---\
                      > @zerocode-ai/tui -> @zerocode-ai/sdk
packages/cli --------/
```
...
`packages/cli`, or `@zerocode-ai/core`.
...
- Do not replace private `packages/opencode` imports with `@zerocode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@zerocode-ai/tui`.

## Ownership Boundary

### `@zerocode-ai/tui` Owns
...
Status: Completed. The private `@zerocode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@zerocode-ai/tui`.
...
- The package has no dependency on `opencode`, `@zerocode-ai/cli`, or
  `@zerocode-ai/core`.
...
primitives now live in `@zerocode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@zerocode-ai/tui/runtime`. Movable app, component,
...
`@zerocode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@zerocode-ai/tui`. Configured references resolve through the new
...
`@zerocode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@zerocode-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@zerocode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@zerocode-ai/core`, or either
  executable package.
...
rg '@zerocode-ai/core|packages/opencode|packages/cli' packages/tui
