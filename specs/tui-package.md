...
```text
packages/opencode ---\
                      > @0codeai/zerocode-tui -> @0codeai/zerocode-sdk
packages/cli --------/
```
...
`packages/cli`, or `@0codeai/zerocode-core`.
...
- Do not replace private `packages/opencode` imports with `@0codeai/zerocode-core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@0codeai/zerocode-tui`.

## Ownership Boundary

### `@0codeai/zerocode-tui` Owns
...
Status: Completed. The private `@0codeai/zerocode-tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@0codeai/zerocode-tui`.
...
- The package has no dependency on `opencode`, `@0codeai/zerocode-cli`, or
  `@0codeai/zerocode-core`.
...
primitives now live in `@0codeai/zerocode-tui`. The legacy host consumes explicit
...
and build inputs through `@0codeai/zerocode-tui/runtime`. Movable app, component,
...
`@0codeai/zerocode-tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@0codeai/zerocode-tui`. Configured references resolve through the new
...
`@0codeai/zerocode-tui`. The legacy host injects a narrow plugin host that retains
...
public `@0codeai/zerocode-tui` root while retaining worker/server/config/plugin and
...
- Change those adapters to load config, create transport inputs, and call the
  public `@0codeai/zerocode-tui` API.
...
- Narrow `@0codeai/zerocode-tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@0codeai/zerocode-core`, or either
  executable package.
...
rg '@0codeai/zerocode-core|packages/opencode|packages/cli' packages/tui
